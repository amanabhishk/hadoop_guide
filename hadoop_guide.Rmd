...
1. Place [this script](https://raw.githubusercontent.com/amanabhishk/hadoop_guide/master/hsubmit) in the same folder. <mark>The easiest way to do that might be opening the link in your browser and then saving the webpage (shortcut is <kbd>command</kbd>+<kbd>s</kbd> on Mac). Name the file `hsubmit`, but *do not worry about the extension - there should be no extension*.  </mark>
...
1. <mark>You can check that your job's output is there or not by doing `hadoop fs -ls`. You should see `gun` when you do so, and the corresponding listed date of file creation should make sense. `gun` is the raw output which consists of many tiny files - it is not a regular csv file, which is what we want.</mark>
1. <mark>The next step is to use `getmerge` command to convert the raw `gun` to `gun.csv`, where the latter would be the final output. Do do this, do `hadoop fs -getmerge gun gun.csv`. </mark>
1. <mark>If you did `ls` now, you will see `gun.csv` in the cs directory. `gun.csv` contains the data from your job, ready for analysis and publication!</mark>
...
<mark>Note 2: The general structure of `getmerge` command is `hadoop fs -getmerge rawoutputXYZ final.csv`, where the name `final` is of your choosing, and `rawoutputXYZ` is the name of the raw output.</mark>
