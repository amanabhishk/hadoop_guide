...
Some thing to look up and learn, but can be skipped now:

1. How do you delete a folder that is empty, or non-empty?
1. Can you `ls` somewhere without doing `cd` into that location first? Same question for removing and copying files.
1. What is `scp`?
...
1. Place [this script](https://raw.githubusercontent.com/amanabhishk/hadoop_guide/master/hsubmit) in the same folder. *Do not worry about the extension - there should be no extension*.
1. Open the script in some text editor - TextEdit/Sublime Text on Mac would work. Here too, change `your_username_goes_here` to your own username (like `aabhishek`). *There is only one place - line 13 - which you will be editing in this step.*
1. Back in your terminal, enter `chmod +x hsubmit`. This gives the computer the permission to run `hsubmit` as a script. 
...
1. Do `hadoop fs -getmerge gun gun.csv` <kbd>Enter</kbd>. Here, `-getmerge` tells hadoop to merge the tiny files into one file. `gun` is that raw output which consists of many tiny files. `gun.csv` is combined csv file, and you can name this anything - `anything.csv` would work too.
1. If you did `ls` now, you will see `gun.csv` in the cs directory.

Note: `gun` is the name of the raw output from the pull. The name of the raw output is mentioned in the top line of `test.sql`. You could change this in the SQL file, and that would change the output name.
...
In the following sections, you'd learn (in brief) how to write your own jobs, and managing jobs that are running.
...
Data on hadoop exists in different "tables" (files). It is important to know which time period is contained in a table, so that we can decide whether to look in it or not. The contents of the tables can change ([Alex Hanna](http://alex-hanna.com/) maintains this so its up to her), but it happens rarely. To get information regarding this, you can do `hdfs dfs -ls /user/ahanna/gh*` from the hadoop server.
