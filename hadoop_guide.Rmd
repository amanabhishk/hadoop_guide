...
1. Place [this script](https://raw.githubusercontent.com/amanabhishk/hadoop_guide/master/hsubmit) in the same folder. Name the file `hsubmit`, but *do not worry about the extension - there should be no extension*. The easiest way to do that might be opening the link in your browser and then saving the webpage. 
...
1. Back in your terminal, navigate to the folder folder you created in the first step, where `hsubmit` is. Enter `chmod +x hsubmit`. This gives the computer the permission to run `hsubmit` as a script. 
